...
def _grammar():
    ...
    rel_expr = binop_level(add_expr, 'none', 'relational expression',
                           rel_less ^ rel_greater ^ rel_lessequal ^ rel_greaterequal ^ rel_equal ^ rel_notequal)
    ...
    # statement grammar
    stmt = pp.Forward()
    block = (lbra_ + pp.ZeroOrMore(stmt) + rbra_).setParseAction(
        a(lambda l, t: nql.Block(lineno=l, children=list(t))))
    st_whileloop = (while_ - lpar_ - expr - rpar_ - block).setParseAction(
        a(lambda l, t: nql.WhileLoop(lineno=l, children=[t[1], t[2]])))
    ...
    else_elsif = (elsif_ - lpar_ - expr - rpar_ - block - else_chain).setParseAction(
        a(lambda l, t: nql.IfThen(lineno=l, children=[t[1], t[2], t[3]])))
    else_chain << (else_else ^ else_elsif ^ else_none)

    st_if = (if_ - lpar_ - expr - rpar_ - block - else_chain).setParseAction(
        a(lambda l, t: nql.IfThen(lineno=l, children=[t[1], t[2], t[3]])))
    st_assignment = (identifier_ + equal_ + expr + semi_).setParseAction(
        a(lambda l, t: nql.Assign(lineno=l, children=[nql.Reg(lineno=l, name=t[0]), t[2]])))
    arglist = (lpar_ + pp.Optional(pp.delimitedList(identifier_)) + rpar_).setParseAction(lambda t: [list(t)])
    st_proccall = (identifier_ + arglist + semi_).setParseAction(
        a(lambda l, t: nql.Call(lineno=l, func=t[0],
                                children=[nql.Reg(lineno=l, name=arg) for arg in t[1]])))
    ...
    case_arm = (case_ - integer_ - colon_ - pp.ZeroOrMore(stmt)).setParseAction(
        a(lambda l, t: nql.SwitchArm(lineno=l, case=t[1], children=list(t)[2:])))
    def_arm = (default_ - colon_ - pp.ZeroOrMore(stmt)).setParseAction(
        a(lambda l, t: nql.SwitchArm(lineno=l, case=None, children=list(t)[1:])))
    ...
    # top level
    procdef = (proc_ - identifier_ - arglist - block).setParseAction(
        a(lambda l, t: nql.ProcDef(lineno=l, name=t[1], parameters=t[2], children=[t[3]])))
    globaldef = (global_ - identifier_ - semi_).setParseAction(
        a(lambda l, t: nql.GlobalReg(lineno=l, name=t[1])))
    optiondef = (option_ - identifier_ - semi_).setParseAction(
        a(lambda l, t: nql.Option(lineno=l, name=t[1])))
    decl = procdef | globaldef | optiondef
    program = pp.ZeroOrMore(decl).setParseAction(
        a(lambda l, t: nql.Program(lineno=l, children=list(t))))
