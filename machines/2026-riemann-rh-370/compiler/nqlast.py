...
class Reg(NatExpr):
    ...
    def emit_nat_op(self, state, target, _args):
        # A dedicated save register makes every register-copy emit the same
        # pair of transfer subroutines for a given (source, target), letting
        # the dispatcher share copy sites that previously differed only in
        # which scratch register the allocator happened to hand out.
...
class SubEmitter:
    ...
    def get_temp(self):
        if self._scratch_free:
            # Canonical allocation: always hand out the lowest-numbered free
            # scratch register.  LIFO reuse made identical operations at
            # different sites grab different temps, emitting duplicate
            # transfer subroutines that the dispatcher could not share.
