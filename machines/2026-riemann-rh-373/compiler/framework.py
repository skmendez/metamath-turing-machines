...
MAIN_INSERT_NOPS = {}
MAIN_RAW_PART_COUNT = 0
JUMP_CANONICAL = False
...
class MachineBuilder:
    ...
    def makesub(self, *parts, name):
        ...
        if name == 'main()':
            ...
            global MAIN_RAW_PART_COUNT
            MAIN_RAW_PART_COUNT = len(parts)
            if MAIN_INSERT_NOPS:
                expanded = []
                for ix, part in enumerate(parts):
                    expanded.extend([self.noop(0)] * MAIN_INSERT_NOPS.get(ix, 0))
                    expanded.append(part)
                expanded.extend([self.noop(0)] * MAIN_INSERT_NOPS.get(len(parts), 0))
        ...
        canonical_jump = {}
        if JUMP_CANONICAL:
            # Choose one (order, rel) encoding per goto by greedy set cover,
            # minimizing the number of distinct jump subroutines and making
            # textually repeated fragments use identical gadgets.
            valids = {}
            pos = 0
            for idx, part in enumerate(real_parts):
                if isinstance(part, Goto):
                    target = label_offsets[part.name]
                    v = []
                    for jo in range(order + 1):
                        rel = target - ((pos >> jo) << jo)
                        if 0 <= rel < (1 << (jo + 1)):
                            v.append((jo, rel))
                    valids[idx] = v
                pos += part.size
            while valids:
                ...
                best = max(sorted(count), key=lambda pr: (count[pr], -pr[0], -pr[1]))
                for idx in [i for i, v in valids.items() if best in v]:
                    canonical_jump[idx] = best
                    del valids[idx]
        ...
        for pindex, part in enumerate(real_parts):
            if isinstance(part, Goto):
                ...
                if self.options.relative_jumps:
                    ...
                    jump_order, rel = canonical_jump[pindex]
